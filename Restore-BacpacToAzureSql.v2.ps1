...
.DESCRIPTION
    ...
    Version 2 includes improved storage network state tracking and more resilient
    managed private endpoint approval handling.
...
.PARAMETER UseNetworkIsolation
    When specified, forces the import to use network isolation even when public connectivity
    would also work. This submits the import through the REST API and requests managed private
    endpoints for the SQL Server and Storage Account.

.PARAMETER MaxSizeBytes
    Max database size in bytes. Default: 5GB.

.PARAMETER NonInteractive
    Runs without prompts. Required decisions must be supplied via switches and credentials
    must already be provided as parameters.

.PARAMETER AllowTemporaryStorageNetworkChanges
    In non-interactive mode, allows the script to temporarily enable storage public network
    access and/or open the storage firewall when required for the import path.

.PARAMETER OverwriteExistingDatabase
    In non-interactive mode, allows the script to drop and recreate an existing target database.

.PARAMETER ValidateBacpac
    Downloads the BACPAC and performs structural and compatibility validation.
    Use together with -PreflightOnly to test a BACPAC without starting an import.

.PARAMETER ExpectedHash
    Optional SHA256 hash to verify after downloading the BACPAC during validation.

.PARAMETER SanitizeAwsRdsBacpac
    Downloads the source BACPAC, removes selected AWS RDS-specific users, logins,
    and triggers from model.xml, uploads a sanitized BACPAC back to blob storage,
    and imports the sanitized copy.

.PARAMETER RemoveDatabaseUsers
    Database user names or wildcard patterns to remove from the BACPAC model.
    Default: sqladmin, rdsadmin.

.PARAMETER RemoveServerLogins
    Login names or wildcard patterns to remove from the BACPAC model.
    Default: sqladmin, rdsadmin.

.PARAMETER RemoveTriggerNames
    Trigger names or wildcard patterns to remove from the BACPAC model.
    Default: rds*.

.PARAMETER SanitizedBacpacSuffix
    Suffix appended to the source blob name when uploading a sanitized BACPAC.
    Default: -sanitized.

.PARAMETER OverwriteSourceBacpac
    When used with -SanitizeAwsRdsBacpac, uploads the sanitized package over the
    original blob instead of creating a separate blob.

.PARAMETER SkipPEAutoApproval
    When specified, skips automatic approval of managed private endpoints during network
    isolation imports. Use this when you don't have permissions for PE approval or prefer
    to approve PEs manually. The import will wait for the PEs to be approved by another
    process or user.
...
.EXAMPLE
    ...
    param(
        ...
        [Parameter()]
        [string]$ScriptPath = ".\Restore-BacpacToAzureSql.v2.ps1"
    )
...
param(
    ...
    [Parameter()]
    [switch]$AllowTemporaryStorageNetworkChanges,

    [Parameter()]
    [switch]$OverwriteExistingDatabase,
    ...
    [Parameter()]
    [switch]$SanitizeAwsRdsBacpac,

    [Parameter()]
    [string[]]$RemoveDatabaseUsers = @('sqladmin', 'rdsadmin'),

    [Parameter()]
    [string[]]$RemoveServerLogins = @('sqladmin', 'rdsadmin'),

    [Parameter()]
    [string[]]$RemoveTriggerNames = @('rds*'),

    [Parameter()]
    [string]$SanitizedBacpacSuffix = '-sanitized',

    [Parameter()]
    [switch]$OverwriteSourceBacpac,

    [Parameter()]
    [switch]$SkipPEAutoApproval
)

Set-StrictMode -Version Latest
$ErrorActionPreference = "Stop"

if ($SkipNetworkIsolation -and $UseNetworkIsolation) {
    throw "-SkipNetworkIsolation and -UseNetworkIsolation cannot be used together. Choose one connectivity mode."
}
...
function Approve-ManagedPrivateEndpoint {
    ...
    $matchingPEs = @(Get-AzPrivateEndpointConnection -PrivateLinkResourceId $ResourceId -ErrorAction SilentlyContinue |
        Where-Object $matchingPeFilter)
    $pendingPEs = @($matchingPEs | Where-Object { $_.PrivateLinkServiceConnectionState.Status -eq 'Pending' })
    $approved = $false
    ...
    $approvedPEs = @($matchingPEs | Where-Object { $_.PrivateLinkServiceConnectionState.Status -eq 'Approved' })
    if ($approvedPEs) {
        $approvedNames = ($approvedPEs | ForEach-Object { ($_.PrivateEndpoint.Id -split '/')[-1] } | Select-Object -Unique) -join ', '
        Write-Host "  [$($ElapsedStr.Value)] $Label PE already in Approved state: $approvedNames" -ForegroundColor Green
        return [PSCustomObject]@{
            IsApproved       = $true
            WasNewlyApproved = $false
        }
    }
...
function Confirm-ExecutionChoice {
    <#
    .SYNOPSIS
        Handles interactive confirmations and enforces explicit behavior in non-interactive mode.
    #>
    param(
        [string]$Prompt,
        [string[]]$ApprovedResponses = @('y', 'Y', 'yes'),
        [string]$NonInteractiveMessage,
        [switch]$AllowInNonInteractive
    )

    if ($NonInteractive) {
        if ($AllowInNonInteractive) {
            return $true
        }
    ...
    return $response -in $ApprovedResponses
}

function Resolve-AdminCredentials {
    <#
    .SYNOPSIS
        Resolves administrator username/password from parameters or prompts.
    #>
    param(
    ...
    if ($AuthenticationType -eq 'ADPassword') {
        ...
        if (-not $SqlAdminUser) {
            if ($NonInteractive) {
                throw "SqlAdminUser is required in -NonInteractive mode for ADPassword authentication."
            }
        ...
        if (-not $SqlAdminUser) {
            if ($NonInteractive) {
                throw "SqlAdminUser is required in -NonInteractive mode for SQL authentication."
            }
    ...
    if (-not $SqlAdminPassword) {
        if ($NonInteractive) {
            throw "SqlAdminPassword is required in -NonInteractive mode."
        }
...
function Resolve-StorageAuthentication {
    <#
    .SYNOPSIS
        Resolves the storage credential payload used by the import operation.
    #>
    ...
    if ($useSas) {
        ...
        $storageCredential = $sasToken.TrimStart('?')
...
function Test-BacpacStructure {
    <#
    .SYNOPSIS
        Validates BACPAC ZIP structure and performs Azure SQL DB compatibility checks.
    #>
    param(
        [Parameter(Mandatory)]
    ...
    try {
        ...
        try {
            ...
            if ($modelEntry) {
                ...
                $result.Messages += "Security principals: $($result.UserCounts.SqlUsers) users, $($result.UserCounts.Roles) roles, $($result.UserCounts.Logins) logins"
                ...
                if ($result.UserCounts.SqlUsers -gt $result.UserCounts.Logins + 2) {
...
function Invoke-BacpacValidation {
    param(
        [Parameter(Mandatory)]
    ...
    try {
        Write-Host "    Downloading '$BlobName' to temp file..." -ForegroundColor White
        ...
        if ($ExpectedHash) {
            ...
            if ($actualHash -eq $ExpectedHash.ToUpperInvariant()) {
                ...
                Write-Warning "      Expected: $($ExpectedHash.ToUpperInvariant())"
                Write-Warning "      Actual:   $actualHash"
                $continue = Confirm-ExecutionChoice -Prompt '    Continue anyway? (y/N)' `
                    -NonInteractiveMessage 'Hash verification failed in -NonInteractive mode. Remove -ExpectedHash or provide the correct value to continue.'
                if (-not $continue) {
        ...
        if (-not $validation.IsValid) {
            ...
            $continue = Confirm-ExecutionChoice -Prompt '    Continue with import anyway? (y/N)' `
                -NonInteractiveMessage 'BACPAC validation failed in -NonInteractive mode. Remove -ValidateBacpac or fix the package before continuing.'
            if (-not $continue) {
        ...
        if ($validation.HasTdeIndicators) {
            ...
            Write-Warning '    The import may fail unless the database was exported with TDE disabled or the original keys are available.'
            $continue = Confirm-ExecutionChoice -Prompt '    Continue anyway? (y/N)' `
                -NonInteractiveMessage 'TDE indicators were found in -NonInteractive mode. Re-run without -ValidateBacpac if you want to bypass this stop.'
            if (-not $continue) {
...
function ConvertTo-NormalizedBacpacName {
    param([string]$Name)
...
function Get-BacpacNameCandidates {
    param([string]$Name)
    ...
    $normalizedName = ConvertTo-NormalizedBacpacName -Name $Name
    $leafName = if ($normalizedName -match '\.') { ($normalizedName -split '\.')[-1] } else { $normalizedName }

    foreach ($candidate in @($Name.Trim(), $normalizedName, $leafName, "[$leafName]")) {
...
function Test-BacpacNameMatch {
    ...
    $candidateNames = Get-BacpacNameCandidates -Name $ObjectName
    foreach ($pattern in $Patterns) {
        if ([string]::IsNullOrWhiteSpace($pattern)) {
            continue
        }

        $normalizedPattern = ConvertTo-NormalizedBacpacName -Name $pattern
        foreach ($candidateName in $candidateNames) {
            $normalizedCandidate = ConvertTo-NormalizedBacpacName -Name $candidateName
...
function Test-BacpacXmlContainsAnyToken {
    param(
        [string]$XmlText,
        [string[]]$Tokens
    )

    if ([string]::IsNullOrWhiteSpace($XmlText) -or -not $Tokens) {
        return $false
    }
    ...
    foreach ($token in $Tokens) {
        ...
        if ($XmlText.IndexOf($token, [System.StringComparison]::OrdinalIgnoreCase) -ge 0) {
            return $true
        }
    }
...
function Get-SanitizedBacpacBlobName {
    param([string]$BlobName)

    if ($OverwriteSourceBacpac) {
        return $BlobName
    }

    $extension = [System.IO.Path]::GetExtension($BlobName)
    $baseName = if ($extension) {
        $BlobName.Substring(0, $BlobName.Length - $extension.Length)
    } else {
        $BlobName
    }

    return "$baseName$SanitizedBacpacSuffix$extension"
}

function Invoke-BacpacSanitization {
    <#
    .SYNOPSIS
        Downloads, edits, and uploads a sanitized BACPAC for Azure SQL import.
    #>
    param(
        [Parameter(Mandatory)]
        [object]$StorageContext,
    ...
    $workingRoot = Join-Path $env:TEMP ("bacpac-sanitize-" + [Guid]::NewGuid().ToString('N'))
    $downloadPath = Join-Path $workingRoot 'source.bacpac'
    $extractPath = Join-Path $workingRoot 'expanded'
    $outputPath = Join-Path $workingRoot 'sanitized.bacpac'

    $removedElements = [System.Collections.Generic.List[object]]::new()
    $referenceTokens = [System.Collections.Generic.HashSet[string]]::new([System.StringComparer]::OrdinalIgnoreCase)
    $removedReferenceNodeCount = 0

    try {
        $null = New-Item -ItemType Directory -Path $workingRoot -Force
        $null = New-Item -ItemType Directory -Path $extractPath -Force

        Write-Host "`nSanitizing BACPAC for Azure SQL compatibility..." -ForegroundColor Cyan
        Write-Host "  Source blob: $SourceBlobName" -ForegroundColor White
        Write-Host "  Target blob: $TargetBlobName" -ForegroundColor White

        Get-AzStorageBlobContent -Container $ContainerName -Blob $SourceBlobName `
            -Context $StorageContext -Destination $downloadPath -Force | Out-Null

        [System.IO.Compression.ZipFile]::ExtractToDirectory($downloadPath, $extractPath)

        $modelPath = Join-Path $extractPath 'model.xml'
        if (-not (Test-Path $modelPath -PathType Leaf)) {
            throw "The BACPAC does not contain model.xml. Sanitization cannot continue."
        }

        $modelXml = New-Object System.Xml.XmlDocument
        $modelXml.PreserveWhitespace = $true
        $modelXml.Load($modelPath)

        foreach ($node in @($modelXml.SelectNodes("//*[local-name()='Element']"))) {
            $typeAttribute = $node.Attributes['Type']
            $nameAttribute = $node.Attributes['Name']
            if (-not $typeAttribute -or -not $nameAttribute) {
                continue
            }

            $elementType = $typeAttribute.Value
            $elementName = $nameAttribute.Value
            $category = $null

            if ($elementType -match 'User' -and (Test-BacpacNameMatch -ObjectName $elementName -Patterns $UserPatterns)) {
                $category = 'User'
            } elseif ($elementType -match 'Login' -and (Test-BacpacNameMatch -ObjectName $elementName -Patterns $LoginPatterns)) {
                $category = 'Login'
            } elseif ($elementType -match 'Trigger' -and (Test-BacpacNameMatch -ObjectName $elementName -Patterns $TriggerPatterns)) {
                $category = 'Trigger'
            }
            ...
            $removedElements.Add([PSCustomObject]@{
                Category = $category
                Type     = $elementType
                Name     = $elementName
            }) | Out-Null

            foreach ($token in Get-BacpacNameCandidates -Name $elementName) {
                $null = $referenceTokens.Add($token)
            }
        ...
        foreach ($node in @($modelXml.SelectNodes("//*[local-name()='Element']"))) {
            ...
            if ($typeAttribute.Value -notmatch 'Permission|RoleMembership|Credential|ExecutionContext|Authorization') {
                continue
            }

            if (Test-BacpacXmlContainsAnyToken -XmlText $node.OuterXml -Tokens @($referenceTokens)) {
                $node.ParentNode.RemoveChild($node) | Out-Null
                $removedReferenceNodeCount++
            }
        }

        foreach ($localName in @('Relationship', 'Entry', 'References')) {
            foreach ($node in @($modelXml.SelectNodes("//*[local-name()='$localName']"))) {
                if (Test-BacpacXmlContainsAnyToken -XmlText $node.OuterXml -Tokens @($referenceTokens)) {
                    $node.ParentNode.RemoveChild($node) | Out-Null
                    $removedReferenceNodeCount++
                }
        ...
        $xmlWriter = [System.Xml.XmlWriter]::Create($modelPath, $xmlWriterSettings)
        try {
            $modelXml.Save($xmlWriter)
        } finally {
            $xmlWriter.Dispose()
        }
        ...
        Set-AzStorageBlobContent -File $outputPath -Container $ContainerName -Blob $TargetBlobName `
            -Context $StorageContext -Force | Out-Null

        return [PSCustomObject]@{
            TargetBlobName            = $TargetBlobName
            RemovedElements           = @($removedElements)
            RemovedReferenceNodeCount = $removedReferenceNodeCount
        }
    } finally {
...
function Invoke-ImportRequest {
    <#
    .SYNOPSIS
        Submits the import request via REST API or cmdlet.
    #>
    ...
    try {
        if ($useRestApi) {
            ...
            return [PSCustomObject]@{ OperationStatusLink = $operationLink; IsRestApi = $true }
        }
...
function Watch-ImportOperation {
    <#
    .SYNOPSIS
        Monitors the import operation and handles managed PE approval plus cleanup.
    #>
    param($ImportRequest)
    ...
    if ($useNetworkIsolation) {
        if ($SkipPEAutoApproval) {
            Write-Host "  PE Auto-Approval: DISABLED - managed PEs must be approved manually or by another process" -ForegroundColor Yellow
        } else {
    ...
    do {
        ...
        if ($useNetworkIsolation -and $elapsed.TotalMinutes -lt 10 -and -not $SkipPEAutoApproval) {
            try {
                if (-not $sqlManagedPeApproved) {
                    $sqlApprovalState = Approve-ManagedPrivateEndpoint -ResourceId $sqlServer.ResourceId -Label 'SQL Server' `
                        -ImportRequestId $importRequestId -ElapsedStr ([ref]$elapsedStr) -ImportStartTime $importStartTime
                    if ($sqlApprovalState.IsApproved) { $sqlManagedPeApproved = $true }
                }
                if (-not $storageManagedPeApproved) {
                    $storageApprovalState = Approve-ManagedPrivateEndpoint -ResourceId $storageAccount.Id -Label 'Storage' `
                        -ImportRequestId $importRequestId -ElapsedStr ([ref]$elapsedStr) -ImportStartTime $importStartTime
                    if ($storageApprovalState.IsApproved) { $storageManagedPeApproved = $true }
                }
    ...
    $timedOut = $status.Status -eq 'InProgress'
    Restore-StorageNetworkSettings -RG $StorageResourceGroupName -AccountName $StorageAccountName `
        -RestoreFirewall $restoreFirewall -RestorePNA $restorePublicNetworkAccess `
...
if ($sqlServer.PublicNetworkAccess -ne "Disabled") {
    ...
    if ($allowAzureServices) {
        ...
        if (-not $sqlPrivateEndpointId) {
            if (-not (Confirm-ExecutionChoice -Prompt "  Do you want to continue anyway? (y/N)" `
                -NonInteractiveMessage "SQL firewall confirmation is required. Either enable the Azure services firewall rule first or run interactively.")) {
...
if ($existingDb -and $existingDb.DatabaseName -ne 'master') {
    ...
    $overwrite = $OverwriteExistingDatabase -or (Confirm-ExecutionChoice -Prompt "  Do you want to DROP and recreate it from the bacpac? (yes/N)" `
        -ApprovedResponses @('yes') -NonInteractiveMessage "Database overwrite requires -OverwriteExistingDatabase when -NonInteractive is used.")
    if (-not $overwrite) {
...
$networkRuleSet = Get-AzStorageAccountNetworkRuleSet -ResourceGroupName $StorageResourceGroupName -AccountName $StorageAccountName
...
if ($storageAccount.PublicNetworkAccess -eq 'Disabled') {
    ...
    if ($SkipNetworkIsolation) {
        $enablePna = $AllowTemporaryStorageNetworkChanges -or (Confirm-ExecutionChoice -Prompt "  Temporarily enable PublicNetworkAccess for the import? It will be restored after. (y/N)" `
            -NonInteractiveMessage "Temporary storage network changes require -AllowTemporaryStorageNetworkChanges when -NonInteractive is used.")
        if ($enablePna) {
            ...
            # Refresh objects after network mutations so later checks use current state.
            $storageAccount = Get-AzStorageAccount -ResourceGroupName $StorageResourceGroupName -Name $StorageAccountName
            $networkRuleSet = Get-AzStorageAccountNetworkRuleSet -ResourceGroupName $StorageResourceGroupName -AccountName $StorageAccountName
...
if ($networkRuleSet.DefaultAction -eq "Deny") {
    ...
    if ($trustedServices -match "AzureServices") {
        ...
        if (-not (Confirm-ExecutionChoice -Prompt "  Do you want to continue anyway? (y/N)" `
            -NonInteractiveMessage "Storage trusted-services confirmation is required. Fix the storage rules first or run interactively.")) {
...
if ($UseNetworkIsolation) {
    ...
    Write-Host "`nNetwork isolation mode: FORCED (user requested -UseNetworkIsolation)" -ForegroundColor Cyan
    if ($sqlPrivateEndpointId) {
    ...
    if (-not $sqlPrivateEndpointId -and -not $storagePrivateEndpointId) {
        Write-Host "  No existing private endpoints detected. The import service will create managed private endpoints for the transfer." -ForegroundColor Yellow
    }
} elseif ($SkipNetworkIsolation) {
...
if ($sqlPrivateEndpointId -or $storagePrivateEndpointId) {
    ...
    if ($useNetworkIsolation) {
        Write-Host "  The import will use network isolation. Azure's import service creates its own" -ForegroundColor White
...
if ($BacpacFileName) {
    ...
    try {
        ...
        if ($ValidateBacpac) {
            try {
                Invoke-BacpacValidation -StorageContext $storageContext -ContainerName $StorageContainerName -BlobName $BacpacFileName
            } catch {
                if ($_.Exception.Message -match 'validation failed|aborted|Hash verification|TDE encryption') {
                    throw
                }
...
return [PSCustomObject]@{
    ...
    SqlPrivateEndpointId        = $sqlPrivateEndpointId
    StoragePrivateEndpointId    = $storagePrivateEndpointId
    UseNetworkIsolation         = $useNetworkIsolation
    RestorePublicNetworkAccess  = $restorePublicNetworkAccess
    RestoreFirewall             = $restoreFirewall
    OriginalBypass              = $originalBypass
    StorageKey                  = $storageKey
}
}

$preflightState = Invoke-PreflightChecks
$currentContext = $preflightState.CurrentContext
$sqlServer = $preflightState.SqlServer
$storageAccount = $preflightState.StorageAccount
$networkRuleSet = $preflightState.NetworkRuleSet
$sqlPrivateEndpointId = $preflightState.SqlPrivateEndpointId
$storagePrivateEndpointId = $preflightState.StoragePrivateEndpointId
$useNetworkIsolation = $preflightState.UseNetworkIsolation
$restorePublicNetworkAccess = $preflightState.RestorePublicNetworkAccess
$restoreFirewall = $preflightState.RestoreFirewall
$originalBypass = $preflightState.OriginalBypass
$storageKey = $preflightState.StorageKey
...
$adminCredentials = Resolve-AdminCredentials -ServerAdminLogin $serverAdminLogin -EntraAdmin $entraAdmin
$SqlAdminUser = $adminCredentials.User
$SqlAdminPassword = $adminCredentials.Password

if ($SanitizeAwsRdsBacpac) {
    $sanitizedBlobName = Get-SanitizedBacpacBlobName -BlobName $BacpacFileName
    $sanitizationContext = New-AzStorageContext -StorageAccountName $StorageAccountName -StorageAccountKey $storageKey
    $sanitizationResult = Invoke-BacpacSanitization -StorageContext $sanitizationContext `
        -ContainerName $StorageContainerName -SourceBlobName $BacpacFileName -TargetBlobName $sanitizedBlobName `
        -UserPatterns $RemoveDatabaseUsers -LoginPatterns $RemoveServerLogins -TriggerPatterns $RemoveTriggerNames

    $removedUsers = @($sanitizationResult.RemovedElements | Where-Object Category -eq 'User')
    $removedLogins = @($sanitizationResult.RemovedElements | Where-Object Category -eq 'Login')
    $removedTriggers = @($sanitizationResult.RemovedElements | Where-Object Category -eq 'Trigger')

    Write-Host "  Sanitization removed $($sanitizationResult.RemovedElements.Count) primary object(s)" -ForegroundColor Green
    Write-Host "    Users: $($removedUsers.Count), Logins: $($removedLogins.Count), Triggers: $($removedTriggers.Count)" -ForegroundColor White
    if ($sanitizationResult.RemovedReferenceNodeCount -gt 0) {
        Write-Host "    Reference nodes removed: $($sanitizationResult.RemovedReferenceNodeCount)" -ForegroundColor White
    }
    if ($sanitizationResult.RemovedElements.Count -eq 0) {
        Write-Warning "  No matching AWS RDS objects were found in model.xml. The sanitized BACPAC is structurally unchanged."
    }

    $BacpacFileName = $sanitizationResult.TargetBlobName
    Write-Host "  Import will use sanitized BACPAC blob: $BacpacFileName" -ForegroundColor Green
}
...
$storageAuth = Resolve-StorageAuthentication
$storageKeyType = $storageAuth.StorageKeyType
$storageCredential = $storageAuth.StorageCredential
...
if ($networkRuleSet.DefaultAction -eq "Deny" -and -not $useNetworkIsolation) {
    ...
    $openFw = $AllowTemporaryStorageNetworkChanges -or (Confirm-ExecutionChoice -Prompt "  Temporarily set storage firewall to 'Allow' for the import? It will be restored after. (y/N)" `
        -NonInteractiveMessage "Temporary storage firewall changes require -AllowTemporaryStorageNetworkChanges when -NonInteractive is used.")
    if ($openFw) {
...
$importRequest = Invoke-ImportRequest
$null = Watch-ImportOperation -ImportRequest $importRequest
